...
async function generateContent(analysisId) {
  ...
  const utmUrls = {
    ...
    facebook: config.utmUrl(analysis.slug, "facebook"),
    tiktok: config.utmUrl(analysis.slug, "tiktok"),
  };
  ...
  // The AI is asked for all four platforms but may omit one; merge each
  // new platform over the deterministic fallback so a package is never
  // missing, then force the platform-specific tracking URL.
  const base = ai.fallbackSocial({ match, slug: analysis.slug, utmUrls });
  ...
  const content = await store.saveContent({
    ...
    facebook: { ...base.facebook, ...(social.facebook || {}), tracking_url: utmUrls.facebook },
    tiktok: { ...base.tiktok, ...(social.tiktok || {}), tracking_url: utmUrls.tiktok },
